...
class EvaluateRecall:
    ...
    def __init__(
        ...
        self.clustered_vcf_query = os.path.abspath(
            outprefix + ".query.filter.cluster.vcf"
        )
        self.clustered_vcf_truth = os.path.abspath(
            outprefix + ".truth.filter.cluster.vcf"
        )
        ...
        self.exclude_regions = EvaluateRecall._load_exclude_regions_bed_file(
    ...
    def _write_vars_plus_flanks_to_fasta(
        cls, outfile, vcf_records, ref_seqs, flank_length, ref_only=False, number_ns=0
    ):
    ...
    def _map_seqs_to_seqs(cls, seqs_file_ref, seqs_file_query, outfile):
        ...
        command = " ".join(
            [bwa_binary, "aln", seqs_file_ref, seqs_file_query, ">", outfile + ".tmp"]
        )
        utils.syscall(command)
        ...
        for sam_record in samfile_handle.fetch(until_eof=True):
            ...
            if match == "Good":
                ...
                for i, vcf_record in enumerate(
                    vcf_reader.fetch(ref_name, vcf_interval_start, vcf_interval_end)
                ):
                    if i == int(vcf_pos_index):
                        sample_name = vcf_record.samples.keys()[0]
                        if (
                            "GT" in vcf_record.format.keys()
                            and len(set(vcf_record.samples[sample_name]["GT"])) == 1
                        ):
    ...
    def _parse_sam_files(
        ...
        query_found, query_conf, query_allele, query_match_flag, query_allele_flag = EvaluateRecall._parse_sam_file_and_vcf(
            samfile,
            query_vcf_file,
            flank_length,
            allow_mismatches,
    ...
    def run(self):
        ...
        if self.filter_and_cluster_vcf:
            EvaluateRecall._filter_vcf_for_clustering(
                self.truth_vcf_file, self.filtered_truth_vcf, self.discard_ref_calls
            )
            EvaluateRecall._filter_vcf_for_clustering(
                self.query_vcf_file, self.filtered_query_vcf, self.discard_ref_calls
            )
            if self.discard_ref_calls:
                clusterer_query = vcf_clusterer.VcfClusterer(
                    [self.filtered_query_vcf],
                    self.query_vcf_ref,
        ...
        vcf_header, vcf_records_truth = vcf_file_read.vcf_file_to_dict(
            self.vcf_to_check_truth, sort=True, remove_useless_start_nucleotides=True
        )
        vcf_header, vcf_records_query = vcf_file_read.vcf_file_to_dict(
            self.vcf_to_check_query, sort=True, remove_useless_start_nucleotides=True
        )
        ...
        EvaluateRecall._write_vars_plus_flanks_to_fasta(
            self.seqs_out_truth,
            vcf_records_truth,
            truth_vcf_ref_seqs,
            self.flank_length,
        ...
        EvaluateRecall._map_seqs_to_seqs(
            self.seqs_out_query, self.seqs_out_truth, self.sam_file_out
        )
        ...
        EvaluateRecall._parse_sam_files(
            self.vcf_to_check_truth,
            self.sam_file_out,
            self.vcf_to_check_query,
            self.sam_summary,
            self.flank_length,
