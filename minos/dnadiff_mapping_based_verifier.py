...
class DnadiffMappingBasedVerifier:
    ...
    def __init__(
        ...
        exclude_regions_bed_file1=None,
        exclude_regions_bed_file2=None,
        max_soft_clipped=3,
    ):
        ...
        self.exclude_regions1 = DnadiffMappingBasedVerifier._load_exclude_regions_bed_file(
            exclude_regions_bed_file1
        )
        self.exclude_regions2 = DnadiffMappingBasedVerifier._load_exclude_regions_bed_file(
            exclude_regions_bed_file2
        )
        self.max_soft_clipped = max_soft_clipped
        self.number_ns = 1

    @classmethod
    def _write_dnadiff_plus_flanks_to_fastas(
        cls,
        dnadiff_file,
        ref_infile,
        query_infile,
    ...
    def _write_vars_plus_flanks_to_fasta(
        cls, outfile, vcf_records, ref_seqs, flank_length, number_ns=0
    ):
    ...
    def _parse_sam_file_and_vcf(
        ...
        for sam_record in samfile_handle.fetch(until_eof=True):
            ...
            for ref_name in exclude_regions.keys():
                ...
                exclude = DnadiffMappingBasedVerifier._interval_intersects_an_interval_in_list(
            ...
            match = DnadiffMappingBasedVerifier._check_if_sam_match_is_good(
                sam_record,
                dnadiff_file_seqs,
                flank_length,
                query_sequence=sam_record.query_sequence,
    ...
    def _parse_sam_files(
        ...
        flank_length,
        allow_mismatches=True,
        exclude_regions1=None,
        exclude_regions2=None,
        max_soft_clipped=3,
        number_ns=0,
    ):
        ...
        snps = pd.read_csv(dnadiff_file, sep="\t", header=None)
        ref_found, ref_conf, ref_allele, ref_match_flag, ref_allele_flag = DnadiffMappingBasedVerifier._parse_sam_file_and_vcf(
            samfile1,
            vcffile1,
            reffasta1,
            flank_length,
        ...
        query_found, query_conf, query_allele, query_match_flag, query_allele_flag = DnadiffMappingBasedVerifier._parse_sam_file_and_vcf(
            samfile2,
            vcffile2,
            reffasta2,
            flank_length,
    ...
    def _gather_stats(cls, tsv_file):
        ...
        for index, line in snps.iterrows():
            ...
            if line["ref_found"] == "Exclude" or line["query_found"] == "Exclude":
                ...
                gt_confs = [
                    i
                    for i in {line["ref_conf"], line["query_conf"]}
                    if not math.isnan(i)
                ]
                gt_conf = None
                if len(gt_confs) > 0:
    ...
    def run(self):
        ...
        vcf_header, vcf_records1 = vcf_file_read.vcf_file_to_dict(
            self.vcf_to_check1, sort=True, remove_useless_start_nucleotides=True
        )
        vcf_header, vcf_records2 = vcf_file_read.vcf_file_to_dict(
            self.vcf_to_check2, sort=True, remove_useless_start_nucleotides=True
        )
        ...
        DnadiffMappingBasedVerifier._map_seqs_to_seqs(
            self.seqs_out_vcf1, self.seqs_out_dnadiff1, self.sam_file_out1
        )
        DnadiffMappingBasedVerifier._map_seqs_to_seqs(
