...
def sam_reader(infile):
    ...
    for sam_record in samfile.fetch(until_eof=True):
        ref_name, expected_start, vcf_record_index, allele_index = sam_record.query_name.rsplit(
            ".", maxsplit=3
        )
...
class MappingBasedVerifier:
    ...
    def __init__(
        ...
        flank_length=31,
        merge_length=None,
        expected_variants_vcf=None,
        run_dnadiff=True,
        filter_and_cluster_vcf=True,
        discard_ref_calls=True,
        allow_flank_mismatches=True,
        exclude_regions_bed_file=None,
        max_soft_clipped=3,
    ):
        ...
        if self.run_dnadiff and self.expected_variants_vcf is not None:
            raise Exception(
                "Error! Incompatible options. expected_variants_vcf file provided, and run_dnadiff is True"
            )
        self.dnadiff_outprefix = os.path.abspath(outprefix + ".dnadiff")
        self.vcf_false_negatives_file_out = os.path.abspath(
            outprefix + ".false_negatives.vcf"
        )
        ...
        self.exclude_regions = MappingBasedVerifier._load_exclude_regions_bed_file(
            exclude_regions_bed_file
        )
    ...
    def _add_edit_distances_to_vcf_record(cls, record):
        ...
        edit_distances = [
            MappingBasedVerifier._edit_distance_between_seqs(record.REF, x)
            for x in record.ALT
        ]
        ...
        with open(outfile, "w") as f:
            ...
            for ref_name in vcf_records:
                for vcf_record in vcf_records[ref_name]:
                    ...
                    if (
                        ...
                        if called_alleles == {"0"}:
                            vcf_record.set_format_key_value(
                                "GT_CONF",
                                str(float(likelihoods[0]) - float(likelihoods[1])),
                            )
                        else:
                            vcf_record.set_format_key_value(
                                "GT_CONF",
                    ...
                    if (
                        "SupportFraction" in vcf_record.INFO.keys()
                        and "GT_CONF" not in vcf_record.FORMAT.keys()
                    ):
                        vcf_record.set_format_key_value(
                            "GT_CONF",
    ...
    @classmethod
    def _write_vars_plus_flanks_to_fasta(
        cls, outfile, vcf_records, ref_seqs, flank_length
    ):
        ...
        with open(outfile, "w") as f:
            for ref_name in sorted(vcf_records):
                for i, vcf_record in enumerate(vcf_records[ref_name]):
                    start_position, alleles = vcf_record.inferred_var_seqs_plus_flanks(
                        ref_seqs[ref_name], flank_length
                    )
    ...
    @classmethod
    def _check_if_sam_match_is_good(
        cls,
        sam_record,
        ref_seqs,
        flank_length,
        ...
        if not allow_mismatches:
            ...
            all_mapped = (
                len(sam_record.cigartuples) == 1 and sam_record.cigartuples[0][0] == 0
            )
        ...
        if (
            sam_record.cigartuples[0][0] == 4
            and sam_record.cigartuples[0][1] > max_soft_clipped
        ) or (
            sam_record.cigartuples[-1][0] == 4
            and sam_record.cigartuples[-1][1] > max_soft_clipped
        ):
        ...
        if len(query_sequence) < 2 * flank_length + 1:
            ...
            if sam_record.query_alignment_start < 5:
                ...
                alt_seq_end = max(
                    alt_seq_start, length_diff + len(query_sequence) - flank_length - 1
                )
        else:
            alt_seq_start = flank_length
            alt_seq_end = len(query_sequence) - flank_length - 1
        ...
        for pair in wanted_aligned_pairs:
            if (
                None in pair
                or query_sequence[pair[0]]
                != ref_seqs[sam_record.reference_name][pair[1]]
            ):
                logging.debug(
                    "SAM record failed because mismatch in allele sequence plus 1bp either side"
                )
    ...
    @classmethod
    def _parse_sam_file_and_update_vcf_records_and_gather_stats(
        cls,
        infile,
        vcf_records,
        flank_length,
        ...
        for sam_list in sreader:
            ...
            for sam_record in sam_list:
                vcfref_name, expected_start, vcf_record_index, allele_index = sam_record.query_name.rsplit(
                    ".", maxsplit=3
                )
                vcf_probe_info_tuples.add(
                    (vcfref_name, expected_start, vcf_record_index)
                )
            ...
            for allele_sam_list in sam_records_by_allele:
                ...
                for i in range(len(allele_sam_list)):
                    if allele_sam_list[i].is_unmapped:
                        ...
                        if truth_name in exclude_regions:
                            ...
                            exclude = MappingBasedVerifier._interval_intersects_an_interval_in_list(
                                interval, exclude_regions[truth_name]
                            )

                        if exclude:
                            match_result_types.add("Exclude")
                        else:
                            match_type = MappingBasedVerifier._check_if_sam_match_is_good(
                                allele_sam_list[i],
                                ref_seqs,
                                flank_length,
                                query_sequence=allele_sam_list[0].query_sequence,
                ...
                if "Good" in match_result_types:
                    ...
                    results["MINOS_CHECK_ALLELES"].append(
                        "Fail-" + "-".join(sorted(list(match_result_types)))
                    )
            ...
            if "GT_CONF" in vcf_record.FORMAT and vcf_record.FORMAT[
                "MINOS_CHECK_GENOTYPE"
            ] in {"0", "1"}:
                tp_or_fp = {"1": "TP", "0": "FP"}[
                    vcf_record.FORMAT["MINOS_CHECK_GENOTYPE"]
                ]
                gt_conf = int(float(vcf_record.FORMAT["GT_CONF"]))
                gt_conf_hists[tp_or_fp][gt_conf] = (
                    gt_conf_hists[tp_or_fp].get(gt_conf, 0) + 1
                )

            # if it's a homozygous call, update the edit distance totals
            if (
                "GT" in vcf_record.FORMAT
                and len(set(vcf_record.FORMAT["GT"].split("/"))) == 1
            ):
                if vcf_record.FORMAT["MINOS_CHECK_GENOTYPE"] == "1":
                    stats["tp_edit_dist"] += int(
                        float(vcf_record.FORMAT["GT_EDIT_DIST"])
                    )
                elif vcf_record.FORMAT["MINOS_CHECK_GENOTYPE"] == "0":
                    stats["fp_edit_dist"] += int(
                        float(vcf_record.FORMAT["GT_EDIT_DIST"])
                    )
    ...
    def _get_missing_vcf_records(cls, called_vcfs, expected_vcfs):
        ...
        for ref_seq in expected_vcfs:
            ...
            for expected_record in expected_vcfs[ref_seq]:
                while (
                    i < len(called_vcf_list)
                    and called_vcf_list[i].ref_end_pos() < expected_record.POS
                ):
                    i += 1
                ...
                if i == len(called_vcf_list):
                    ...
                    called_vcf_list[i].POS > expected_record.POS
                    or expected_record.ref_end_pos() > called_vcf_list[i].ref_end_pos()
                ):
                    missing_records[ref_seq].append(expected_record)
    ...
    @classmethod
    def _get_total_length_of_expected_regions_called(
        cls, expected_regions, called_vcf_records
    ):
        ...
        for ref_seq, expected_list in expected_regions.items():
            if ref_seq in called_vcf_records:
                called_list = [
                    pyfastaq.intervals.Interval(x.POS, x.ref_end_pos())
                    for x in called_vcf_records[ref_seq]
                ]
                ...
                intersect_regions = pyfastaq.intervals.intersection(
                    called_list, expected_list
                )
                called_length += pyfastaq.intervals.length_sum_from_list(
                    intersect_regions
                )
            else:
                total_length += pyfastaq.intervals.length_sum_from_list(expected_list)
    ...
    def run(self):
        ...
        vcf_header, vcf_records = vcf_file_read.vcf_file_to_dict(
            self.vcf_to_check, sort=True, remove_useless_start_nucleotides=True
        )
        ...
        MappingBasedVerifier._map_seqs_to_ref(
            self.seqs_out, self.verify_reference_file, self.sam_file_out
        )
        ...
        if self.run_dnadiff:
            ...
            ] = MappingBasedVerifier._get_total_length_of_expected_regions_called(
