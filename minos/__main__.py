...
def main(args=None):
    ...
    parser.add_argument('--debug', help="More verbose logging, and less file cleaning", action='store_true')
    ...
    subparser_adjudicate.add_argument('--gramtools_kmer_size', type=int, help='This number is used with gramtools build --kmer-size. Ignored if --gramtools_build_dir used.  [%(default)s]', default=10, metavar='INT')
    ...
    subparser_adjudicate.add_argument('--total_splits', type=int, help='Split VCF, aiming for this many chunks with the same number of variants in each chunk. Increases run time, but saves RAM (see also --variants_per_split and --alleles_per_split). If used, then reads must be in one sorted indexed BAM file', metavar='INT')
    ...
    subparser_adjudicate.add_argument('--alleles_per_split', type=int, help='Split VCF, aiming for this many alleles in each split. Takes precedence over --total_splits. Increases run time, but saves RAM. If used, then reads must be in one sorted indexed BAM file', metavar='INT')
    subparser_adjudicate.add_argument('--use_unmapped_reads', action='store_true', help='When using splitting (with --total_splits, --variants_per_split, or --alleles_per_split), use the unmapped reads with each split. Default is to ignore them. Using this option may add huge increase to run time, with little benefit to variant call accuracy')
    ...
    subparser_check_with_ref.add_argument('--variant_merge_length', type=int, help='Variants up to this distance apart are merged, unless they conflict with each other [%(default)s]', default=31, metavar='INT')
    subparser_check_with_ref.add_argument('--no_flank_mismatches', action='store_true', help='Do not allow mismatches in flanking regions when mapping to the truth reference. By default, inexact matches are allowed')
    subparser_check_with_ref.add_argument('--no_filter_cluster', action='store_true', help='Do not filter and cluster the input VCF before checking accuracy')
    subparser_check_with_ref.add_argument('--include_ref_calls', action='store_true', help='Do not filter ref calls from input VCF before checking accuracy')
    subparser_check_with_ref.add_argument('--exclude_bed', help='BED file of regions of the truth reference to ignore (col1=seq name, col2=start, col3=end)', metavar='FILENAME')
    subparser_check_with_ref.add_argument('--max_soft_clipped', type=int, help='Maximum number of bases allowed to be softclipped when parsing SAM file.[%(default)s]', default=3)
    ...
    subparser_check_snps.add_argument('--flank_length', type=int, help='Number of nucleotides to use before and after each allele for mapping [%(default)s]', default=31, metavar='INT')
    subparser_check_snps.add_argument('--variant_merge_length', type=int, help='Variants up to this distance apart are merged, unless they conflict with each other [%(default)s]', default=31, metavar='INT')
    subparser_check_snps.add_argument('--allow_flank_mismatches', action='store_true', help='Allow mismatches in flanking regions when mapping to the truth reference. By default, only exact matches allowed')
    subparser_check_snps.add_argument('--no_filter_cluster', action='store_true', help='Do not filter and cluster the input VCF before checking accuracy')
    subparser_check_snps.add_argument('--include_ref_calls', action='store_true', help='Do not filter ref calls from input VCF before checking accuracy')
    subparser_check_snps.add_argument('--exclude_bed1', help='BED file of regions of the truth reference 1 to ignore (col1=seq name, col2=start, col3=end)', metavar='FILENAME')
    subparser_check_snps.add_argument('--exclude_bed2', help='BED file of regions of the truth reference 2 to ignore (col1=seq name, col2=start, col3=end)', metavar='FILENAME')
    subparser_check_snps.add_argument('--max_soft_clipped', type=int, help='Maximum number of bases allowed to be softclipped when parsing SAM file.[%(default)s]', default=3)
    subparser_check_snps.add_argument('dnadiff_snps_file', help='Name of SNPs file to be checked')
    subparser_check_snps.add_argument('dnadiff_ref', help='FASTA of truth reference for first sample in dnadiff SNPs file')
    subparser_check_snps.add_argument('dnadiff_query', help='FASTA of truth reference for second sample in dnadiff SNPs file')
    ...
    subparser_check_snps.add_argument('vcf_ref', help='Reference FASTA file used to make VCF (must match VCF file)')
    ...
    subparser_check_recall.add_argument('--flank_length', type=int, help='Number of nucleotides to use before and after each allele for mapping [%(default)s]', default=31, metavar='INT')
    subparser_check_recall.add_argument('--variant_merge_length', type=int, help='Variants up to this distance apart are merged, unless they conflict with each other [%(default)s]', default=31, metavar='INT')
    subparser_check_recall.add_argument('--allow_flank_mismatches', action='store_true', help='Allow mismatches in flanking regions when mapping to the truth reference. By default, only exact matches allowed')
    subparser_check_recall.add_argument('--no_filter_cluster', action='store_true', help='Do not filter and cluster the input VCF before checking accuracy')
    subparser_check_recall.add_argument('--include_ref_calls', action='store_true', help='Do not filter ref calls from input VCF before checking accuracy')
    subparser_check_recall.add_argument('--exclude_bed', help='BED file of regions of the truth reference to ignore (col1=seq name, col2=start, col3=end)', metavar='FILENAME')
    subparser_check_recall.add_argument('--max_soft_clipped', type=int, help='Maximum number of bases allowed to be softclipped when parsing SAM file.[%(default)s]', default=3)
    subparser_check_recall.add_argument('truth_vcf_file', help='Name of VCF file to be checked')
    subparser_check_recall.add_argument('truth_vcf_ref', help='FASTA of truth reference for truth VCF file')
    subparser_check_recall.add_argument('query_vcf_file', help='VCF file')
    subparser_check_recall.add_argument('query_vcf_ref', help='Reference FASTA file used to make VCF (must match VCF file)')
    ...
    subparser_make_split_gramtools_build.add_argument('outdir', help='Name of output directory')
    subparser_make_split_gramtools_build.add_argument('vcf_file', help='Input VCF file to be split')
    subparser_make_split_gramtools_build.add_argument('ref_fasta', help='Reference FASTA file')
    subparser_make_split_gramtools_build.add_argument('--total_splits', type=int, help='Split VCF, aiming for this many chunks with the same number of variants in each chunk (see also --variants_per_split and --alleles_per_split) [%(default)s]', metavar='INT', default=100)
    subparser_make_split_gramtools_build.add_argument('--variants_per_split', type=int, help='Split VCF, aiming for this many variants in each split. If used, --total_splits is ignored', metavar='INT')
    subparser_make_split_gramtools_build.add_argument('--alleles_per_split', type=int, help='Split VCF, aiming for this many alleles in each split. If used, --total_splits is ignored', metavar='INT')
    subparser_make_split_gramtools_build.add_argument('--max_read_length', type=int, help='This number is used with gramtools build --max-read-length [%(default)s]', default=200)
    subparser_make_split_gramtools_build.add_argument('--gramtools_kmer_size', type=int, help='This number is used with gramtools build --kmer-size [%(default)s]', default=10, metavar='INT')
    subparser_make_split_gramtools_build.add_argument('--threads', type=int, help='Number of gramtools builds to run in parallel [%(default)s]', default=1, metavar='INT')
    subparser_make_split_gramtools_build.set_defaults(func=minos.tasks.make_split_gramtools_build.run)
    ...
    subparser_multi_sample_pipeline.add_argument('--max_alleles_per_cluster', type=int, help='Maximum allowed alleles in one cluster. If there are too many alleles then combinations of SNPs are not generated [%(default)s]', metavar='INT', default=5000)
    ...
    subparser_multi_sample_pipeline.add_argument('--gramtools_kmer_size', type=int, help='This number is used with gramtools build --kmer-size [%(default)s]', default=10, metavar='INT')
    subparser_multi_sample_pipeline.add_argument('--gramtools_build_threads', type=int, help='Number of gramtools builds to run in parallel (only used if --total_splits,--variants_per_split, or --alleles_per_split used) [%(default)s]', default=1, metavar='INT')
    subparser_multi_sample_pipeline.add_argument('--total_splits', type=int, help='Split VCF, aiming for this many chunks with the same number of variants in each chunk. Increases run time, but saves RAM (see also --variants_per_split and --alleles_per_split). If used, then reads for each sample must be in one sorted indexed BAM file', metavar='INT')
    subparser_multi_sample_pipeline.add_argument('--variants_per_split', type=int, help='Split VCF, aiming for this many variants in each split. Takes precedence over --total_splits. Increases run time, but saves RAM. If used, then reads for each sample must be in one sorted indexed BAM file', metavar='INT')
    subparser_multi_sample_pipeline.add_argument('--alleles_per_split', type=int, help='Split VCF, aiming for this many alleles in each split. Takes precedence over --total_splits. Increases run time, but saves RAM. If used, then reads for each sample must be in one sorted indexed BAM file', metavar='INT')
    subparser_multi_sample_pipeline.add_argument('--use_unmapped_reads', action='store_true', help='When using splitting (with --total_splits, --variants_per_split, or --alleles_per_split), use the unmapped reads with each split. Default is to ignore them. Using this option may add huge increase to run time, with little benefit to variant call accuracy')
