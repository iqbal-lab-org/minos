...
class TestGramtools(unittest.TestCase):
    ...
    def test_update_vcf_record_using_gramtools_allele_depths_heterozygous(self):
        ...
        expected = vcf_record.VcfRecord(
                "ref\t4\t.\tT\tA,G,TC\t.\t.\tKMER=42\tGT:DP:COV:GT_CONF\t0/2:17:9,0,7,0:54.46"
        )
        mean_depth = 15
        error_rate = 0.001
        kmer_size = 42
        ...
        expected_filtered = vcf_record.VcfRecord(
                "ref\t4\t.\tT\tG\t.\t.\tKMER=42\tGT:DP:COV:GT_CONF\t0/1:17:9,7:54.46"
        )
    ...
    def test_update_vcf_record_using_gramtools_allele_depths_homozygous(self):
        ...
        expected = vcf_record.VcfRecord(
                "ref\t4\t.\tT\tTC,G\t.\t.\tKMER=42\tGT:DP:COV:GT_CONF\t2/2:81:1,0,80:87.29"
        )
        mean_depth = 85
        error_rate = 0.001
        kmer_size = 42
        ...
        self.assertEqual(expected, record)
        expected_filtered = vcf_record.VcfRecord(
                "ref\t4\t.\tT\tG\t.\t.\tKMER=42\tGT:DP:COV:GT_CONF\t1/1:81:1,80:87.29"
        )
        self.assertEqual(expected_filtered, got_filtered)
    ...
    def test_update_vcf_record_using_gramtools_allele_depths_not_callable(self):
        ...
        expected = vcf_record.VcfRecord(
                "ref\t4\t.\tT\tG,TC\t.\t.\tKMER=42\tGT:DP:COV:GT_CONF\t./.:0:0,0,0:0.0"
        )
        mean_depth = 85
        error_rate = 0.001
        kmer_size = 42
