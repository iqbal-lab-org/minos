...
class TestPlots(unittest.TestCase):
    def test_load_dp_and_gt_conf_data_from_file(self):
        expect_dict = {"DP": [3, 6, 10, 42], "GT_CONF": [5.0, 10.0, 9.0, 42]}
        ...
        expect_df = pd.DataFrame(data=expect_dict)
        infile = os.path.join(
            data_dir, "load_dp_and_gt_conf_data_from_file.with_tp_fp.tsv"
        )
        got = plots.load_dp_and_gt_conf_data_from_file(infile)
        pd.util.testing.assert_frame_equal(expect_df, got)
    ...
    def test_minos_vcf_to_plot_data(self):
        ...
        infile = os.path.join(
            data_dir, "minos_vcf_to_plot_data.partial_check_geno.in.vcf"
        )
        expect_file = os.path.join(
            data_dir, "minos_vcf_to_plot_data.partial_check_geno.expect.tsv"
        )
        ...
        expect_file = os.path.join(
            data_dir, "minos_vcf_to_plot_data.no_check_geno.expect.tsv"
        )
        ...
        infile = os.path.join(
            data_dir, "minos_vcf_to_plot_data.partial_dp_and_gt_conf.in.vcf"
        )
        expect_file = os.path.join(
            data_dir, "minos_vcf_to_plot_data.partial_dp_and_gt_conf.expect.tsv"
        )
        self.assertEqual(set(), plots.minos_vcf_to_plot_data(infile, tmpfile))
        self.assertTrue(filecmp.cmp(expect_file, tmpfile, shallow=False))
        os.unlink(tmpfile)
        ...
        infile = os.path.join(
            data_dir, "minos_vcf_to_plot_data.no_dp_and_gt_conf.in.vcf"
        )
    ...
    def test_plots_from_minos_vcf(self):
        ...
        expect_files = [
            outprefix + x + ".pdf"
            for x in [".gt_conf_dp_scatter", ".dp_hist", ".gt_conf_hist"]
        ]
