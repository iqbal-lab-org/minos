...
class TestMappingBasedVerifier(unittest.TestCase):
    def test_needleman_wunsch(self):
        ...
        seq1 = "ACGTGTCACAG"
        seq2 = "AGTCTGACATG"
        aln1, aln2 = mapping_based_verifier.MappingBasedVerifier._needleman_wunsch(
            seq1, seq2
        )
    ...
    def test_edit_distance_from_alignment_strings(self):
        ...
        self.assertEqual(
            0,
            mapping_based_verifier.MappingBasedVerifier._edit_distance_from_alignment_strings(
                "A", "A"
            ),
        )
        self.assertEqual(
    ...
    def test_edit_distance_between_seqs(self):
        ...
        self.assertEqual(
            0,
            mapping_based_verifier.MappingBasedVerifier._edit_distance_between_seqs(
                "A", "A"
            ),
        )
        self.assertEqual(
    ...
    def test_add_edit_distances_to_vcf_record(self):
        ...
        vcf = vcf_record.VcfRecord(
            "ref1\t3\tid_1\tTCGA\tTCCA,TAAA\t42.42\tPASS\tKMER=31;SVLEN=0;SVTYPE=SNP\tGT:COV:GT_CONF\t2/2:0,52:39.80:1"
        )
        mapping_based_verifier.MappingBasedVerifier._add_edit_distances_to_vcf_record(
            vcf
        )
    ...
    def test_load_exclude_regions_bed_file(self):
        ...
        self.assertEqual(
            {},
            mapping_based_verifier.MappingBasedVerifier._load_exclude_regions_bed_file(
                None
            ),
        )
        got = mapping_based_verifier.MappingBasedVerifier._load_exclude_regions_bed_file(
            tmp_bed
        )
    ...
    def test_interval_intersects_an_interval_in_list(self):
        ...
        for interval, true_or_false in tests:
            self.assertEqual(
                true_or_false,
                mapping_based_verifier.MappingBasedVerifier._interval_intersects_an_interval_in_list(
    ...
    def test_filter_vcf_for_clustering(self):
        ...
        mapping_based_verifier.MappingBasedVerifier._filter_vcf_for_clustering(
            vcf_in, tmp_out
        )
    ...
    def test_filter_vcf_for_clustering_keep_ref_calls(self):
        ...
        expected_vcf = os.path.join(
            data_dir, "filter_vcf_for_clustering.with_ref_calls.expect.vcf"
        )
        tmp_out = "tmp.filter_vcf_for_clustering.out.vcf"
        discard_ref_calls = False
        mapping_based_verifier.MappingBasedVerifier._filter_vcf_for_clustering(
            vcf_in, tmp_out, discard_ref_calls
        )
        self.assertTrue(filecmp.cmp(expected_vcf, tmp_out, shallow=False))
        os.unlink(tmp_out)
    ...
    def test_map_seqs_to_ref(self):
        ...
        if os.path.exists(tmp_out):
            os.unlink(tmp_out)
        mapping_based_verifier.MappingBasedVerifier._map_seqs_to_ref(
            reads, ref, tmp_out
        )
    ...
    def test_check_if_sam_match_is_good(self):
        ...
        for sam_record in samfile.fetch(until_eof=True):
            expected = {"yes": "Good", "no": "Bad"}[
                sam_record.query_name.split(".")[-1]
            ]
            got = mapping_based_verifier.MappingBasedVerifier._check_if_sam_match_is_good(
                sam_record, ref_seqs, 29, allow_mismatches=True
            )
        ...
        for sam_record in samfile.fetch(until_eof=True):
            if first:
                self.assertTrue(
                    mapping_based_verifier.MappingBasedVerifier._check_if_sam_match_is_good(
                        sam_record, ref_seqs, 29, allow_mismatches=False
                    )
                )
                first = False
            else:
    ...
    def test_check_called_genotype(self):
        ...
        vcf = vcf_record.VcfRecord(
            "ref\t42\t.\tT\tA,G\t42.0\t.\tDP4=42\tGT:MINOS_CHECK_ALLELES\t0/0:Bad,Bad,Bad"
        )
        expect_vcf = vcf_record.VcfRecord(
            "ref\t42\t.\tT\tA,G\t42.0\t.\tDP4=42\tGT:MINOS_CHECK_ALLELES:MINOS_CHECK_GENOTYPE\t0/0:Bad,Bad,Bad:0"
        )
        mapping_based_verifier.MappingBasedVerifier._check_called_genotype(vcf)
        self.assertEqual(expect_vcf, vcf)

        vcf = vcf_record.VcfRecord(
            "ref\t42\t.\tT\tA,G\t42.0\t.\tDP4=42\tGT:MINOS_CHECK_ALLELES\t0/0:Bad,Bad,Pass"
        )
        expect_vcf = vcf_record.VcfRecord(
            "ref\t42\t.\tT\tA,G\t42.0\t.\tDP4=42\tGT:MINOS_CHECK_ALLELES:MINOS_CHECK_GENOTYPE\t0/0:Bad,Bad,Pass:0"
        )
        mapping_based_verifier.MappingBasedVerifier._check_called_genotype(vcf)
        self.assertEqual(expect_vcf, vcf)

        vcf = vcf_record.VcfRecord(
            "ref\t42\t.\tT\tA,G\t42.0\t.\tDP4=42\tGT:MINOS_CHECK_ALLELES\t0/0:Pass,Bad,Bad"
        )
        expect_vcf = vcf_record.VcfRecord(
            "ref\t42\t.\tT\tA,G\t42.0\t.\tDP4=42\tGT:MINOS_CHECK_ALLELES:MINOS_CHECK_GENOTYPE\t0/0:Pass,Bad,Bad:1"
        )
        mapping_based_verifier.MappingBasedVerifier._check_called_genotype(vcf)
        self.assertEqual(expect_vcf, vcf)

        vcf = vcf_record.VcfRecord(
            "ref\t42\t.\tT\tA,G\t42.0\t.\tDP4=42\tGT:MINOS_CHECK_ALLELES\t0/1:Pass,Bad,Bad"
        )
        expect_vcf = vcf_record.VcfRecord(
            "ref\t42\t.\tT\tA,G\t42.0\t.\tDP4=42\tGT:MINOS_CHECK_ALLELES:MINOS_CHECK_GENOTYPE\t0/1:Pass,Bad,Bad:HET"
        )
        mapping_based_verifier.MappingBasedVerifier._check_called_genotype(vcf)
        self.assertEqual(expect_vcf, vcf)

        vcf = vcf_record.VcfRecord(
            "ref\t42\t.\tT\tA,G\t42.0\t.\tDP4=42\tGT:MINOS_CHECK_ALLELES\t1/1:Bad,Pass,Bad"
        )
        expect_vcf = vcf_record.VcfRecord(
            "ref\t42\t.\tT\tA,G\t42.0\t.\tDP4=42\tGT:MINOS_CHECK_ALLELES:MINOS_CHECK_GENOTYPE\t1/1:Bad,Pass,Bad:1"
        )
        mapping_based_verifier.MappingBasedVerifier._check_called_genotype(vcf)
        self.assertEqual(expect_vcf, vcf)

        vcf = vcf_record.VcfRecord(
            "ref\t42\t.\tT\tA,G\t42.0\t.\tDP4=42\tMINOS_CHECK_ALLELES\tBad,Bad,Bad"
        )
        expect_vcf = vcf_record.VcfRecord(
            "ref\t42\t.\tT\tA,G\t42.0\t.\tDP4=42\tMINOS_CHECK_ALLELES:MINOS_CHECK_GENOTYPE\tBad,Bad,Bad:UNKNOWN_NO_GT"
        )
        mapping_based_verifier.MappingBasedVerifier._check_called_genotype(vcf)
        self.assertEqual(expect_vcf, vcf)
    ...
    def test_get_missing_vcf_records(self):
        ...
        got = mapping_based_verifier.MappingBasedVerifier._get_missing_vcf_records(
            vcfs_to_check, vcfs_should_find
        )
        self.assertEqual(expected, got)
    ...
    def test_get_total_length_of_expected_regions_called(self):
        ...
        expected_regions = {
            ...
            "ref.2": [pyfastaq.intervals.Interval(42, 43)],  # 2 long, none get called
        }
